...
### Try it yourself!
Below is the *test_module* function code from [Hello World](https://github.com/demisto/content/tree/master/Packs/HelloWorld/Integrations/HelloWorld) integration, an example package where you can see our conventions at action!
some parts in the code are missing, fill in the parts in the code labeled as `{your code here}` with the correct data.
you can find the full solution in the tab below.

the command we will test is the **search_alerts** command which returns the same autherization error as our Yoda API.

```LearningEditor
original='./tut-integration-editor-files/test_module_template.py'
solution='./tut-integration-editor-files/test_module_solution.py'
```
...
### Try it yourself!
Complete the main function for our *HelloWorld* integration, currently let's only support our **helloworld-search-alerts** command.
Change all the `'{your code here}'` blocks with the right code.

```LearningEditor
original='./tut-integration-editor-files/main_template.py'
solution='./tut-integration-editor-files/main_solution.py'
```
