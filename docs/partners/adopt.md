...
1. Partner opens a PR on the pack. The PR must update the readme (per text below) and increment the version number together with the relevant release note. It is recommended to run `demisto-sdk update-release-notes -i Packs/<MyPack>` which will generate a new release note file and will bump the pack version. More information can be found [here](..docs/documentation/release-notes). This starts the 90 day transition period. 
