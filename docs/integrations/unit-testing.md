...
* When using demistomock, mock the `demisto` object directly using `mocker`, don't rely on the defaults.
...
* When a few lines are repeated in multiple tests as setup, consider using fixtures instead. Learn more about Pytest fixtures [here](https://docs.pytest.org/en/6.2.x/fixture.html).
