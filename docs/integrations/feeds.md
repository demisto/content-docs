...
`set_feed_last_run` - Checks current XSOAR version: prior 6.2.0, this function sets the feed's last run from the `IntegrationContext` using `setIntegrationContext`, and from 6.2.0 and later it sets it from the `lastRun` object, using `setLastRun`.
