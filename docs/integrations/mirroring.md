...
*  **triggerDebugMirroringRun (Available from 6.6)** - is a new cli command runnable through the war room, uses for debugging a full mirroring run over the existing incidents. The command gets an `incidentId`. If no id got, the incident will be loaded from the war-room context. The output of this command is a unique debug log file includes some useful logs about the whole mirroing flow. In addition, this debug log includes the integrations' debug-mode logs content. Note: This command triggers a real mirroring actions, for example, updating incidents and fields.  
