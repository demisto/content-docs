...
*  **triggerDebugMirroringRun (Available from 6.6)** - is runnable through the War Room for debugging a full mirroring run over the existing incidents. The command gets an `incidentId`. If no ID is retrieved, the incident will be loaded from the War Room context. The output of this command is a unique debug log file that includes some useful logs about the entire mirroring flow. In addition, this debug log includes the content of the integrations' debug-mode logs. 
