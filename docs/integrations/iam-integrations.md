...
Creates a user. If the user already exists, updates it. If the user exists but disabled, enables the user.
...
Updates a user. If the user exists but disabled, enables the user.
...
2. **handle_exception()**: Every API has its own error handling. For example, when a user does not exist, we can get an empty 200 response or 404 error response. Because we never don't want to fail the command, we will handle such cases by identifying them and set the results according to the content of ***ERROR_CODES_TO_SKIP*** and the given [IAMActions](#iamactions) object.
