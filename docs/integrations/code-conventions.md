...
### AutoExtract
As part of ```CommandResults()``` there is an argument called ```ignore_auto_extract```, which prevents the built-in auto-extract feature from enriching IPs, URLs, files, and other indicators from the result. For example:
