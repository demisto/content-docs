...
## Troubleshooting
* **Playbook is "stuck" on `Waiting for polling to complete`:** Since generic polling schedule tasks outside the context of the playbook (not visible in the playbook run), a lot of errors appear only in the War Room. Go to the incident War Room and check for errors or warnings related to GenericPolling tasks.
...
* **The integration returnes an ID not found error when running from GenericPolling, however when running the get status command manuualy, it finish siccessfully:** Some products cannot handle consequtive requests for querying an action status right after the request to perform the action itself. After you initiate the action, try adding a waiting mechanism before calling the **GenericPolling** sub-playbook
