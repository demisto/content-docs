...
* **The integration returnes an ID not found error when running from GenericPolling, however when running manuualy, it finishes successfully:** Some products cannot handle consequtive requests for querying an action status right after the request to perform the action itself. After you initiate the action, try adding a `sleep` task before calling the **GenericPolling** sub-playbook.
