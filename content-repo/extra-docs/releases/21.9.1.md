...
#### Integrations
##### ArcusTeam
The ArcusTeam API allows you to inspect the connected devices' attack surface. By feeding device identifiers and the software it runs, DeviceTotal will return a map of the device’s attack surface. DeviceTotal was built from the ground up in order to provide complete visibility into connected devices and mitigate 3rd party risk. DeviceTotal can continuously identify and predict such that the connected device security posture is assessed, prioritized, and mitigated effectively.
...
* Indicators, files and pProcess creation pattern hunting using PAN-OS, Cortex XDR, and SIEM products.
...
Display War Room entries in a dynamic section which are tagged with 'report'.
...
#### Scripts
##### JsonUnescape
Recursively un-escapes JSON data if the escaped JSON is found.
...
Alert triage using Arcanna.Ai Machine Learning capabilities and reinforcement learning by offering analyst feedback for closed incidents. (Available from Cortex XSOAR 5.5.0).

#### Scripts
##### New: ArcannaFeedbackPostProcessing
- Arcanna.Ai post-processing script for sending feedback to Arcanna about the closed incident. Additional modification might be required depending on the Cortex setup.
This script is intended to be used as a sample or in conjunction with the Arcanna-Generic-Investigation-V2-With-Feedback playbook. (Available from Cortex XSOAR 5.5.0).
 - If other fields are used for closing an incident, then those fields should also be sent to the integration.
 - If other post-processing scripts are already in place, then the script can be incorporated.
...
Added the PDF files preview as and image step to the playbook.
...
Lists the Docker images used by the installed integrations and automations.
...
#### Layouts
**Confluera Layout** - Updated the script IDs in the **layoutscontainer-Confluera_Layout** to the relevant script IDs.
...
The playbook checks whether the FireEye Email Security integration is enabled and whether the Domain input has been provided and if so, blocks the domain. (Available from Cortex XSOAR 6.0.0).
...
Added a playbook that uses the details of the compromised credentials ingested from Flashpoint and authenticates using the Active Directory integration by providing the compromised credentials of the user, expires the credentials if it matches, and sends an email alert about the breach.
...
Automatically changes the email field when choosing a classification.
...
Added a classification for the following incident types:
...
Enables you to run XQL queries on your data sources. (Available from Cortex XSOAR 5.5.0).
...
Fixed an issue where the following built-in commands would not execute the desired query:
...
Added the following Wildfire supported file types: 
- ELF
- HTA
- LNK
- VBS
...
The playbook checks whether the Proofpoint Threat Response integration is enabled, and whether the Domain input has been provided, and if so, blocks the domain. (Available from Cortex XSOAR 5.5.0).
...
#### Integrations
##### ServiceNow v2
- When creating notes in XSOAR, the integration displays the username of the user who created the note. However, when an automation creates a note, it shows up as "dbot". Now, this will also happen for instances where the user field is populated with an empty string.
...
#### Integrations
##### Slack v3 (beta)
- **Breaking Change**: Notifications for incidents will no longer be sent by default. To receive notifications regarding an incident, configure the *Types of Incidents* parameter and select the notifications you would like to receive.
- Added the *permitted_notifications* parameter that allows customization of the different notifications for Slack to send.
...
The playbook checks whether the Symantec Messaging Gateway integration is enabled, and whether the Domain input has been provided, and if so, blocks the domain. (Available from Cortex XSOAR 5.5.0).
...
The playbook checks whether the Trend Micro Apex One integration is enabled, and whether the Domain input has been provided, and if so, blocks the domain. (Available from Cortex XSOAR 5.5.0).
...
#### Integrations
##### VMRay
-  Extended the ***vmray-get-sample-by-hash*** command to include the File indicator output.
- Updated the Docker image to: *demisto/python:2.7.18.24066*.
​
...
The playbook checks whether the Zscaler integration is enabled, and whether the Domain input has been provided, and if so, blocks the domain. (Available from Cortex XSOAR 5.5.0).
