...
description: The Phishing Alerts content pack uses out-of-the-box playbooks, to handle phishing alerts recieved by either your Email Security Gateway or SIEM products. 
---

 # Phishing Alerts #

Email Security Gateways produce a high amount of phishing alerts, which are different by the type, severity and handling process. It is important to address these alerts in order to identify campaigns, analyze their IoCs and protect the organization from any malicious payload that was delivered within them.
...
|---------------- | ------------- | ------------- |
| Phishing Alerts Investigation | This is the pack's parent playbook and is the default playbook for the **Phishing Alerts** incident type. Use this playbook to investigate and remediate a potential phishing incident produced by either your Email Security Gateway or SIEM product. One of the playbook's main tasks is retrieving the original email file from your Email Security Gateway or Email Service Provider. The playbook response tasks take under consideration the initial severity, hunting results and also the existence of similar phishing incidents in XSOAR. No action will be taken without an initial approval given by the analyst using the playbook inputs. | Under the playbook inputs, you can add the SOC email address to send the notificiations via email. |
| Phishing Alerts - Check Severity | This playbook is executed as part of the **Phishing Alerts Investigation** parent playbook and is responsible for calculating the incident severity and notify the SOC via email if sensitive mailbox has been detected.| - | 
...
## How to Use the Pack
In order to use this pack, you need to configure several integrations and map and classify your Email Security Gateway or SIEM incidents.

### Before You Start

There are several items that you must install and configure before you start using this pack.

### 1. Configure an integration which produce phishing alerts and allows to fetch them
The main playbook in this pack relies on Email Security Gateways or SIEM phishing alerts. That means you must configure either one of them in order to use the pack.
...
7. Map the relevant fields from the JSON by selecting the keys and clicking **Choose data path**. In order to make this pack work, please map the following fields:
...
8. Map the relevant fields from the JSON by selecting the keys and clicking **Choose data path**. In order to make this pack work, please map the following fields:
...
Process Email - Generic v2 is one of the main sub-playbooks being executed in **Phishing Alerts Investigation** playbook. 
There is one important input you should pay attention to:
...
Knowing the Email Security Gateways produce high amount of alerts, a customer can consider dropping alerts which has an action value of blocked/dropped alert.
