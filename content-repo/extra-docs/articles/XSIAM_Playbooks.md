...
Executing a containment action such as 'Quarantine file' manually will prompt the user with a question already populated with interesting files he might want to quarantine.
...
Executing an eradication action such as 'Kill process' manually will prompt the user with a question already populated with interesting processes he might want to terminate.
...
2. Executes the 'Command-Line Analysis' sub-playbook. This playbook takes the command line from the alert and performs the following actions:
    ...
    1. Checks for base64 string and decodes if exists
...
6. Executes the Containment and Eradication Plan sub-playbooks based on the investigation results.
