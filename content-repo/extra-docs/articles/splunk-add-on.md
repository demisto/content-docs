...
5. A message saying that you need to restart Splunk will appear. Click **Restart Now**.  
...
4. Configure a Cortex XSOAR instance:
   ...
   - Configuration for Cortex XSOAR 8 or Cortex XSIAM instances:
     1. Enter a unique instance name on the *XSOAR Instance name* field.
     2. Enter your Cortex XSOAR server's API URL in the *XSOAR API URL* field.  
        The API URL can be obtained in Cortex XSOAR by navigating to **Settings & Info** > **Settings** > **Integrations** > **API Keys** and clicking **Copy API URL**.  
     3. Enter your API key on the ***API Key*** field.  
...
5. [Optional] Proxy Setup:  
   If you want to tunnel the traffic to the add-on through a proxy, select the **Proxy** tab, check the **Enable** checkbox, and fill all relevant proxy configuration fields.
...
6. [Optional] Log Level Configuration:  
   By default, the logging level is "INFO". If you want to change it (we don't recommend setting it to a value that isn't `INFO` or `DEBUG`), select the **Logging** tab, and choose the desired logging level from the *Log Level* drop-down list.
...
7. [Optional] SSL Certificate Configuration:  
   If you have an internal / self-signed SSL certificate, select the **Additional Settings** tab, and provide its full path in the *Location to Certificate* field.  
...
8. [Optional] Timeout configuration:
   If you would like to extend the incident creation request timeout time, select the **Additional Settings** tab, and enter the desired timeout value in the *Timeout Value* field.  
   By default, timeout value is 10 seconds.
9. After all configurations are set, restart Splunk to apply the changes.  
   To restart Splunk, go to **Settings** > **Server Controls**, and click **Restart Splunk**.
...
3. Click **Select File**, select a random file (preferably a small text-based file), and click **Next**.  
   ![splunk-add-data-upload-data-form.png](../../../docs/doc_imgs/reference/SplunkAddOn/splunk-add-data-upload-data-form.png)
4. Click **Review** at the top, and then **Submit**.
...
6. A "New Search" page will open. Click **Save As**, and then **Alert**.
...
7. Configure the alert:
     ...
     - Under *Trigger Actions*, click **Add Actions**, and then select **Create XSOAR Incident**.
     - Configure the *Create XSOAR Incident* action as described in the [Alert Configuration](#alert-configuration) section.
8. Enter your Cortex XSOAR server, wait a few minutes for incidents to be ingested, and ensure that incidents are being created in Cortex XSOAR properly.
...
2. Click the **Actions** drop-down list for the incident that you want to be created on Cortex XSOAR.
...
Connection timeout errors can be identified in the Splunk logs, and will appear as follows:
```
